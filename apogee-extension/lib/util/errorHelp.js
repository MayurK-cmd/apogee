// Every failure the extension shows gets a link to ERROR.md, deep-linked
// to the section that explains it. Anchors are GitHub's heading slugs, so they
// have to stay in sync with the headings in that file.
...
// Order matters. Internal strings go first so "Unknown ollama-stream action"
// does not land in the Ollama section, and the in-browser patterns beat the
// Ollama ones because the WebLLM offscreen message names Ollama as the fix.
const SECTIONS = [
...
// Falls back to the top of the file when nothing matches, which is also what
// happens for raw browser or library strings that were never written for a user.
