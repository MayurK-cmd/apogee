...
- **Local Ollama connection**: to reach Ollama, Apogee strips the `Origin` header from its `localhost` / `127.0.0.1` requests via a bundled [declarativeNetRequest](apogee-extension/rules/ollama-cors.json) rule (scoped to those loopback hosts only), so Ollama accepts them without any `OLLAMA_ORIGINS` configuration. The rule also excludes `localhost` / `127.0.0.1` as an _initiator_, so it only ever touches Apogee's own requests: a page you have open from a local dev server keeps its `Origin` header, and the CSRF defences of your other local services are left intact. This is a local, on-device request path, not a data-transmission path to any third party. Ollama itself only binds to `127.0.0.1` by default, so it's never reachable from your network regardless.
...
- **Browser Permission Sandboxing**:
  ...
  - **`declarativeNetRequestWithHostAccess`**: Backs the single bundled rule that strips the `Origin` header from loopback Ollama requests (see "Local Ollama connection" above). The rule is scoped to `127.0.0.1`/`localhost` only, excludes requests initiated by local pages so it can't weaken another local service, and this permission grants header rewriting exclusively on hosts the extension already has access to.
